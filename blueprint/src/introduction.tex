...
The inductive type \texttt{Game} has finite lists of options, so every game tree in the
development is finite.  Accordingly, the quotient constructed here is the ring of
...
All nodes marked as formalized were checked against the current local Lean environment.  A
green node means that the named declaration exists; the prose proof explains the mathematical
route actually used by the repository at the level relevant to this blueprint.
