...
\begin{proof}
  \leanok
  \uses{def:game,def:game-order,lem:birthday-options,def:game-inductions,thm:game-order-calculus,thm:option-extensionality,def:game-add}
  ...
  \[
    ...
    \end{aligned}
  \]
  On the left the three sublists are parenthesized as
  \((A^L\mathbin{++}B^L)\mathbin{++}C^L\), whereas on the right they are
  parenthesized as \(A^L\mathbin{++}(B^L\mathbin{++}C^L)\).  Associativity of
  finite-list concatenation first identifies these two parenthesizations.
...
\begin{proof}
  \leanok
  \uses{lem:birthday-options,def:game-order,def:game-inductions,thm:game-order-calculus,def:game-add,thm:game-add-laws}
...
\begin{proof}
  \leanok
  \uses{lem:birthday-options,def:game-order,def:game-inductions,def:game-neg}
  ...
  Applying the displayed equivalence to both weak inequalities in the
  definition of \(\GameEq\) proves the stated biconditional for equivalence.
  Applying it to the forward weak
  inequality and to the excluded reverse inequality proves the strict-order
  equivalence.  Both conclusions use the definitions in
...
\begin{proof}
  \leanok
  \uses{def:game,def:game-order,def:game-inductions,lem:birthday-options,thm:game-order-calculus,def:game-add,thm:game-add-laws,def:game-neg}
  ...
  Definition~\ref{def:game-order}.  Since \(0=\{\,\mid\,\}\) by
...
\begin{definition}[Auxiliary quotient of all games]
  ...
  Let \(\mathcal G/{\GameEq}\) be the quotient of all finite games by game equivalence.  With
  the operations described below, this quotient is a partially ordered additive commutative
  group.  It will be used for algebraic rearrangement in later product proofs; totality is not
  asserted for arbitrary games.
\end{definition}
\begin{proof}
  \leanok
  \uses{def:game-order,thm:game-order-calculus,thm:game-add-laws,thm:game-add-order,thm:game-neg-order,thm:game-add-inverse}
  Write \([a]\) for the equivalence class of a game \(a\).  Theorem~\ref{thm:game-order-calculus}
  shows that \(\GameEq\) is an equivalence relation.  Define
  \[
    [a]+[b]=[a+b],\qquad -[a]=[-a],\qquad [a]\leq[b]
      \Longleftrightarrow a\GameLe b.
  \]
  The first two definitions are independent of the representatives by the
  ...
  \(a\GameEq b\) by Definition~\ref{def:game-order}, so \([a]=[b]\) by the
...
\begin{proof}
  \leanok
  \uses{thm:surreal-totality,thm:game-order-calculus,thm:game-add-laws,thm:game-add-order,thm:game-add-inverse,def:surreal-number}
  By Definition~\ref{def:surreal-number}, an element of \(\NoShort\) is an
  equivalence class \([a]\) of surreal representatives.  Set
  \([a]\leq[b]\) when \(a\GameLe b\).  If either representative is replaced
  by an equivalent one, the two relevant weak inequalities and transitivity
  from Theorem~\ref{thm:game-order-calculus} show that this condition is
  unchanged.  Reflexivity and transitivity descend for the same reason.  Two
  opposite inequalities say \(a\GameEq b\), so the classes are equal by the
  definition of the quotient.  Finally, Theorem~\ref{thm:surreal-totality}
  applies to the surreal representatives and makes this order total.  Hence it
  is a linear order.

  Addition and negation of classes are well defined by
  Definition~\ref{def:surreal-number}.  The zero, associativity, and
  commutativity laws follow from Theorem~\ref{thm:game-add-laws}, while the
  inverse law follows from Theorem~\ref{thm:game-add-inverse}; literal equality
  or game equivalence of representatives gives equality of their classes.
  If \(a\GameLe b\), two-variable monotonicity from
  Theorem~\ref{thm:game-add-order}, applied together with reflexivity of the
  common summand from Theorem~\ref{thm:game-order-calculus}, gives
  \(c+a\GameLe c+b\).  Thus addition is compatible with the quotient order.
\end{proof}
